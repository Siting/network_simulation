...
for k = 1 : length(linkIDs)
    ...
    for i = 1 : numStages
        h(i) = scatter(allAccepted(2,:,i), allAccepted(3,:,i), col(i), 'filled');   % accepted
        hold on
    end
    ...
    saveas(gcf, ['.\Plots\case' num2str(series) '\link' num2str(linkID) '_2D_allAccepted_dmaxdc.pdf']);
    saveas(gcf, ['.\Plots\case' num2str(series) '\link' num2str(linkID) '_2D_allAccepted_dmaxdc.fig']);
    saveas(gcf, ['.\Plots\case' num2str(series) '\link' num2str(linkID) '_2D_allAccepted_dmaxdc.eps'], 'epsc');
end