...
for k = 1 : length(linkIDs)
    ...
    for i = 1 : numStages
        h(i) = scatter(allAccepted(1,:,i), allAccepted(2,:,i), col(i), 'filled');   % accepted
        hold on
    end
    ...
    saveas(gcf, ['.\Plots\case' num2str(series) '\link' num2str(linkID) '_2D_allAccepted_vdmax.pdf']);
    saveas(gcf, ['.\Plots\case' num2str(series) '\link' num2str(linkID) '_2D_allAccepted_vdmax.fig']);
    saveas(gcf, ['.\Plots\case' num2str(series) '\link' num2str(linkID) '_2D_allAccepted_vdmax.eps'], 'epsc');
end