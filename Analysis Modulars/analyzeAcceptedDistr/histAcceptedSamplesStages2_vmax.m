...
for j = 1 : length(studyLinks)
    ...
    for i = 1 : length(studyStages)
        ...
        figure(link) 
        [n, xout] = hist(ACCEPTED_POP(link).samples(1,:));
...
saveas(gcf, ['../Plots\series' num2str(series) '\acceptedSamplesHist2_vmax.pdf']);
saveas(gcf, ['../Plots\series' num2str(series) '\acceptedSamplesHist2_vmax.fig']);
saveas(gcf, ['../Plots\series' num2str(series) '\acceptedSamplesHist2_vmax.eps'], 'epsc');