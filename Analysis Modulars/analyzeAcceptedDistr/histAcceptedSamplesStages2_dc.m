...
for j = 1 : length(studyLinks)
    ...
    for i = 1 : length(studyStages)
        ...
        figure(link) 
        [n, xout] = hist(ACCEPTED_POP(link).samples(3,:));
...
saveas(gcf, ['../Plots\series' num2str(series) '\acceptedSamplesHist2_dc.pdf']);
saveas(gcf, ['../Plots\series' num2str(series) '\acceptedSamplesHist2_dc.fig']);
saveas(gcf, ['../Plots\series' num2str(series) '\acceptedSamplesHist2_dc.eps'], 'epsc');